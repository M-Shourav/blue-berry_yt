...
const SingleProductPage =async ({params}:{params:Promise<{slug:string}>}) => {
  ...
  const product=await getProductBySlug(slug)    
    ...
    <div>
        ...
        {product?.image && (
           <div className='w-full md:w-1/2 border border-blue-300 rounded-md
            group overflow-hidden shadow-md h-auto'>
             <Image src={urlFor(product?.image).url()} alt='productImage'
              width={700}
               height={700}
               loading='lazy'
        ...
        <div className='flex flex-col gap-2 w-full md:w-1/2'>
            ...
            className="text-xl mt-4"
             />
            {product?.stock && (
              <span className='text-base font-semibold w-fit px-4 py-2 rounded-md bg-green-100 text-green-800 my-2'>In Stoke</span>
              )}
              <p className='text-base text-gray-800 my-1'><span className='px-2 py-2 bg-black text-white rounded-md'>20</span> People are viewing this right now </p>
               <h3 className='text-base font-medium mb-2'>{product?.description}</h3>
               {product && <AddToCartButton product={product}/>}

               <div className='mt-2 w-full flex flex-wrap items-center justify-between gap-2.5
               ...
               <div className='w-full flex flex-col md:flex-row items-center justify-between gap-2.5 py-5'>
                <div className='w-full md:w-1/2 border border-gray-400 flex flex-col py-4
                 items-center justify-center rounded-md hover:border-blue-400 cursor-pointer'>
                  ...
                  <p className='text-base text-muted-foreground'>Free shipping over order $120</p>
                </div>
                <div className='w-full md:w-1/2 border border-gray-400 flex flex-col py-4
                 items-center justify-center rounded-md hover:border-blue-400 cursor-pointer'>
                  <h1 className='text-lg font-semibold'>Flexible Payment</h1>
                  <p className='text-base text-muted-foreground'>Pay with Multiple Credit Cards</p>
                </div>
               </div>
        </div>
        </Container>
