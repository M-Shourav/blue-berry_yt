...
const Header = async() => {
  ...
  return (
    <header className='w-full bg-white py-4 border-b border-b-gray-300
     sticky top-0 z-50'>
      <Container className='flex flex-wrap items-center justify-between gap-7'>
        ...
        <input type="text" name="query" placeholder='search name of products...' 
        className='w-full px-4 py-2  border border-gray-300 outline-none rounded-md focus-visible:border-blue-400' />
        </Form>
        <div className='flex items-center gap-2'>
          <CartIcon/>
          <SignedIn>
          <Link href={"/orders"} className='flex items-center gap-2 px-2 py-1 shadow-md
           hover:shadow-none ring-1 ring-gray-100 rounded-sm duration-300'>
           <ShoppingBasket className='w-6 h-6 text-blue-400'/>
          <div className='flex flex-col'>
          <p className='text-xs font-semibold'><span>0</span> items</p>
          <p className='text-sm font-semibold'>Orders</p>
          </div>
          </Link>
          </SignedIn>
          {user ? (
          <div className='flex items-center gap-2 px-2 py-1 shadow-md
          hover:shadow-none ring-1 ring-gray-100 rounded-sm duration-300'>
              <UserButton/>
          ...
          <div className='flex items-center gap-2 px-2 py-1 shadow-md group cursor-pointer
          hover:shadow-none ring-1 ring-gray-100 rounded-sm duration-300 '>
